...
emailjs.init("Bxv-OmZrxtYjIYkWQ"); 
...
// ===========================================
// CONTAINERIZED EXCLUSIVELY ROUTED CONTACT API
// ===========================================
...
contactForm.addEventListener('submit', async function(e) {
    ...
    // Basic frontend verification fallback
    if (!name || !email || !message) {
        formStatus.textContent = 'Please fill in all fields before transmitting.';
        formStatus.className = 'form-status-msg error';
        return;
    }

    // Freeze UI interaction states during delivery
    submitBtn.textContent = 'Sending...';
    submitBtn.disabled = true;

    try {
        // Fire asynchronous call straight to your Docker microservice on Render
        const response = await fetch('https://portfolio-backend-k5n2.onrender.com/api/v1/contact', {
            method: 'POST',
            headers: {
        ...
        if (response.ok && data.status === 'success') {
            // SUCCESS
            formStatus.textContent = '✅ Message sent successfully! Notification routed to email.';
            formStatus.className = 'form-status-msg success';
            contactForm.reset(); // Clear all fields
        } else {
            // FAIL (Triggered by express-validator failure catch rules)
            formStatus.textContent = `❌ Validation Error: ${data.message || 'Transmission rejected.'}`;
            formStatus.className = 'form-status-msg error';
        }
    } catch (error) {
        // BACKEND OFFLINE OR NETWORK DROPPED
        console.error('API Error:', error);
        formStatus.textContent = '❌ Unable to connect to your API server. Please check deployment or email directly.';
        formStatus.className = 'form-status-msg error';
    } finally {
        // Restore interaction loop control parameters
